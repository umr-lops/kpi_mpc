...
def preprocess_L2F(ds, variables=None, add_ecmwf_wind=True):
    ...
    if variables is not None:
        ...
        for kk in ds.keys():
            ...
            if "dataset" in ds[kk].dims:
                ...
                ds[kk + "_ww3"] = xarray.DataArray(
                    ds[kk][:, 1].values, dims=["fdatedt"]
                )  # checked in partition_wv_xassignement.py
                ds[kk + "_s1"] = xarray.DataArray(
                    ds[kk][:, 0].values, dims=["fdatedt"]
                )
    if "ecmwf_wind_speed" not in ds and add_ecmwf_wind:
        tmpval = np.sqrt(
            ds["ecmwf0125_uwind"].values ** 2.0
            + ds["ecmwf0125_vwind"].values ** 2.0
        )
    ...
    ds = ds.assign_coords(
        {"dataset": ["sar", "ww3"]}
    )  # to fix a S1B 20210223 file...
    # logging.info('ds.dims  %s ',ds.dims)
    # logging.info('ds %s',ds)
    return ds


def read_L2F_with_xarray(
    start,
    stop,
    satellites=SAR_UNITS,
    variables=None,
    ...
    for (
        ...
        satellites
    ):  # pas de S1B pour le moment car pas de fichier avec les varaibles cross assigned 28 janvier 2020
        ...
        if start.year == stop.year:
            pat = os.path.join(
                datadir, start.strftime("%Y"), "*", sensor + "*nc"
            )
        else:
        ...
        dates = []
        for ff in listnc0:
            datdt = datetime.datetime.strptime(
                os.path.basename(ff).split("_")[5], "%Y%m%d"
            )

...
def list_input_files(kpi, start, stop):
    ...
    if kpi == "kpi1d":
        logging.info(
            "the version of WV L2F daily data push to ECE is %s",
            DIR_L2F_WV_DAILY,
        )
        for dd in rrule.rrule(rrule.DAILY, dtstart=start, until=stop):
            pattern = os.path.join(
                DIR_L2F_WV_DAILY, dd.strftime("%Y"), dd.strftime("%j"), "*nc"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--kpi", choices=["kpi1b", "kpi1d"], required=True, help="kpi1b kpi1d"
    )
    parser.add_argument(
        "--startdate",
        help="YYYYMMDD",

...
def read_fat_calib_nc(satellite_list=["S1A", "S1B"]):
    ...
    for satellite0 in satellite_list:
        ...
        if os.path.exists(output_file_calibration):
            ...
            logging.debug(
                "var kurt_sar %s", "kurt_sar" in nctest.variables.keys()
            )
            logging.debug(
                "var skew_sar %s", "skew_sar" in nctest.variables.keys()
            )
            nctest.close()
            ...
            df_slc = df_slc[
                (df_slc["_lat_sar"] > -90) & (df_slc["_lat_sar"] < 90)
            ]
            logging.info(
                "after lat filter %s finite %s %s",
                len(df_slc),
                np.isfinite(df_slc["_lat_sar"]).sum(),
                np.isfinite(df_slc["_lon_sar"]).sum(),
            )
            # print(df_slc)
            df_slc.dropna(subset=["time"], inplace=True)
            df_slc["_sig_sar_db"] = 10.0 * np.log10(df_slc["_sig_sar"])
            df_slc["sigma0_denoised_db"] = 10.0 * np.log10(
                df_slc["sigma0_denoised"]
            )
            ...
            logging.info(
                "elapsed time to have the data reade %1.1f seconds"
                % (time.time() - t0)
            )
            df_slc["distance2coast"] = dst
            # add the windazi
            test_trackangle = (df_slc["_tra_sar"]) % 360
            azi_sar_test = compute_wind_azimuth.method_wind_azi_range(
                df_slc["_zon_coloc_ecmwf"],
            ...
            gmf = GMFCmod5n()
            sigma0_cmod5n = gmf._getNRCS(
                df_slc["_inc_sar"].values,
                df_slc["_spd_coloc_ecmwf"].values,
                azi_sar_test,
            )
            df_slc["tmp_gmf_cmod5n_nrcs"] = sigma0_cmod5n
            df_slc["tmp_gmf_cmod5n_nrcs_db"] = 10.0 * np.log10(sigma0_cmod5n)

            # fix ecmwf wind directions
            _dirtest2 = np.mod(
                np.degrees(
                    np.arctan2(
                        df_slc["_zon_coloc_ecmwf"], df_slc["_mer_coloc_ecmwf"]
                    )
                ),
                360.0,
            )
            ...
            print(f"no {output_file_calibration}")
    return df_slc_sat

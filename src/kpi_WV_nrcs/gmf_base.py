...
class GMFBase:
    def __init__(self, name, band, polarization="VV", vmax=25.0):
        self._name = name
        self._band = band
    ...
    def _getWindSpeed(self, inc_angle, sigma0, wind_dir):
        ...
        nb_sol = self.nb_sol
        speed = (
            np.zeros(sigma0.shape + (nb_sol,)) + np.nan
        )  ###nan element (no zero value in averaging)
        dv = 10.0
        ...
        def _iter(pos):
            ...
            if not w.any():
                ...
                speed[pos][0 : min(nb_sol, len(ind))] = v[
                    ind[0 : min(nb_sol, len(ind))]
                ]

            else:
                # case 2: at least 1 zero-crossing points
                v_solut = (
                    np.zeros(w.shape) + np.nan
                )  ### nan elements (no zero value on the averaging)
                for k in np.transpose(w.nonzero()):
                    v_solut[k] = (tab[k + 1] * v[k] - tab[k] * v[k + 1]) / (
                        tab[k + 1] - tab[k]
                    )
                # Only 1 solution
                if np.sum(w) == 1:
                    ...
                    if nb_sol == 1:
                        ...
                        if len(siz) == 2:  # 2D array
                            ...
                            if w2.any():
                                ...
                                w3 = np.isfinite(
                                    speed[yy, xx]
                                )  ### find the not nan value in the neibourghood
                                if w3.any():
                                    tmp = speed[xx, yy]
                                    d = np.nansum(
                                        (
                                        ...
                                        ** 2,
                                        axis=0,
                                    )  ##correction  sum is on the first dimension
                                    ind = np.nanargmin(d)
                            ...
                            if len(siz) == 1:  # 1D array
                                if pos > 0:
                                    ...
                                    if w2:
                                        ind = np.argmin(
                                            (speed[xx] - v_solut) ** 2.0
                                        )
                                        speed[pos] = v_solut[ind]
                    else:

                        speed[pos][0 : max(nb_sol, np.sum(w))] = v_solut[
                            ~np.isnan(v_solut)
                        ][0 : max(nb_sol, np.sum(w))]
    ...
    def getNRCS(
        self, inc_angle, wind_speed, wind_dir, polarization, oPolarizationRatio
    ):
        """
    ...
    def getWindSpeed(
        self, inc_angle, sigma0, wind_dir, polarization, oPolarizationRatio
    ):
        """
        ...
        cSigma0 = oPR.convertSigma0(
            sigma0, inc_angle, wind_dir, polarization, self.polarization
        )
        ...
        if ysize <= tile_size and xsize <= tile_size:
            return np.squeeze(
                self._getWindSpeed(inc_angle, cSigma0, np.abs(wind_dir))
            )

        # traitement par tuile pour ne pas exploser la mémoire
        for sy, sx in itertile(tile_size, 0, 0, xsize, ysize):
            speed[:, sy, sx] = self._getWindSpeed(
                inc_angle[sy, sx], cSigma0[sy, sx], abs(wind_dir[sy, sx])
            )

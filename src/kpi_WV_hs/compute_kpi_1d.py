...
def compute_kpi_1d(sat, wv, dev=False, stop_analysis_period=None):
    ...
    logging.info(
        ...
        np.sum(cond_wind),
        100.0 * np.sum(cond_wind) / cond_wind.size,
    )
    logging.info(
        "nb pts after NRCS filter %s %1.1f%%",
        np.sum(cond_nrcs),
        100.0 * np.sum(cond_nrcs) / cond_nrcs.size,
    )
    ...
    logging.info(
        "nb finite %s/%s",
        np.isfinite(subset_df["bias_swh_azc_" + wv]).sum(),
    ...
    mask_current_month = (
        ocean_acqui_filters
        & (df["fdatedt"] >= start_current_month)
        & (df["fdatedt"] <= stop_current_month)
    )
    ...
    if (
        ...
        nb_measu_outside_envelop = (
            abs(subset_current_period["bias_swh_azc_" + wv]) < envelop_value
        ).sum()

        kpi_value = 100.0 * nb_measu_outside_envelop / nb_measu_total
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--satellite", choices=SAR_UNITS, required=True, help="S-1 unit choice"
    )
    parser.add_argument(
        "--wv",
        choices=["wv1", "wv2"],
    ...
    kpi_v, start_cur_month, stop_cur_month, envelop_val = compute_kpi_1d(
        sat, wv=wv, dev=True, stop_analysis_period=end_date
    )
    logging.info("##########")
    logging.info(

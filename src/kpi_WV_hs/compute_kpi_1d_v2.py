...
MS = {"wv1": -6.5, "wv2": -15.0}  # NRCS central values dB
...
def load_Level2_series(satellite, start, stop, alternative_L2F=None):
    ...
    vv = [
        ...
        "oswEcmwfWindSpeed",
        "oswQualityFlagPartition1",
        "oswQualityFlagPartition2",
        "oswQualityFlagPartition3",
        "oswQualityFlagPartition4",
        "oswQualityFlagPartition5",
        "s1_hs_emp_tot_v3p2",
        "oswXA_hs_ww3spec_firstSARpartition",
        "oswXA_hs_ww3spec_secondSARpartition",
        "oswXA_hs_ww3spec_thirdSARpartition",
        "oswXA_hs_ww3spec_fourthSARpartition",
        "oswXA_hs_ww3spec_fifthSARpartition",
        "oswXA_wl_ww3spec_firstSARpartition",
        "oswXA_wl_ww3spec_secondSARpartition",
        "oswXA_wl_ww3spec_thirdSARpartition",
        "oswXA_wl_ww3spec_fourthSARpartition",
        "oswXA_wl_ww3spec_fifthSARpartition",
    ]
    ds_dict_sat = read_L2F_with_xarray(
        start,
        stop,
    ...
    if dswv != {}:
        # drop Nan
        dswv = dswv.where(
            np.isfinite(dswv["s1_effective_hs_2Dcutoff"])
            & np.isfinite(dswv["ww3_effective_2Dcutoff_hs"]),
            drop=True,
        )
    return dswv
...
def compute_kpi_1d(
    ...
    version,
    dev=False,
    stop_analysis_period=None,
    period_analysed_width=30,
    df=None,
    ...
    start_current_month = stop_current_month - datetime.timedelta(
        days=period_analysed_width
    )
    start_prior_period = start_current_month - datetime.timedelta(
        days=30 * PRIOR_PERIOD
    )
    stop_prior_period = start_current_month
    nb_measu_total = 0
    if df is None:
        df = load_Level2_series(
            satellite=sat,
    ...
    logging.debug(
        "prior period ; %s to %s", start_prior_period, stop_prior_period
    )
    _swh_azc_mod = df["ww3_effective_2Dcutoff_hs"].values
    logging.debug(
        "nb value WW3 eff Hs above 25 m : %s", (_swh_azc_mod > 25).sum()
    )
    if (_swh_azc_mod > 25).sum() > 0:
        ind_bad_ww3 = np.where(_swh_azc_mod > 25)[0][0]
        logging.debug(
            "a date SAR for which ww3 is extremely too high: %s -> Hs:%1.1fm",
            df["fdatedt"][ind_bad_ww3],
    ...
    if version == "test1" or version == "test4" or version == "test5":
        ...
        df["bias_swh_azc_" + wv] = xarray.DataArray(
            ...
            - df["oswXA_hs_ww3spec_firstSARpartition"].values[:, 1],
            dims=["fdatedt"],
            coords={"fdatedt": df["fdatedt"]},
        )
        ...
        df["bias_swh_azc_" + wv] = xarray.DataArray(
            df["oswXA_wl_ww3spec_firstSARpartition"].values[:, 0]
            - df["oswXA_wl_ww3spec_firstSARpartition"].values[:, 1],
            dims=["fdatedt"],
            coords={"fdatedt": df["fdatedt"]},
        )
    else:
    ...
    if wv == "wv1":
        ...
        raise Exception(f"wv value un expected : {wv}")
    ...
    if version == "test1":
        ...
        central_ws = 7
        cond_outlier_ww3_hs = (
            abs(df["ww3_effective_2Dcutoff_hs"]) < 50
        )  # & (abs(df['ww3_effective_2Dcutoff_hs']) > 1)
        cond_wind = np.abs(df["ecmwf_wind_speed"] - central_ws) < delta_ws
        logging.debug(
            "nb pts after wind filter %s %1.1f%%",
            np.sum(cond_wind.values),
            100.0 * np.sum(cond_wind.values) / cond_wind.size,
        )
        logging.debug(
            "nb pts after NRCS filter %s %1.1f%%",
            np.sum(cond_nrcs.values),
        ...
        ocean_acqui_filters = (
            ...
            & (df["oswLandFlag"] == 0)
            & (df["dist2coastKM"] > MIN_DIST_2_COAST)
            & cond_inc
            & (np.isfinite(df["bias_swh_azc_" + wv]))
        )
    elif version == "test2":
        cond_quality = (df["oswQualityFlagPartition1"] == 0) | (
            df["oswQualityFlagPartition1"] == 1
        )  # very good
        ...
        cond_quality = (
            (df["oswQualityFlagPartition1"] == 0)
            | (df["oswQualityFlagPartition1"] == 1)
            | (df["oswQualityFlagPartition1"] == 2)
        )  # very good
        cond_outlier_ww3_hs = abs(df["ww3_effective_2Dcutoff_hs"]) < 50
        logging.debug("cond_quality %s", cond_quality.values.sum())
        fini_bias = np.isfinite(df["bias_swh_azc_" + wv]) & (
            abs(df["bias_swh_azc_" + wv]) < 50
        )
        logging.debug("fini_bias %s", fini_bias.values.sum())
        latmax_cond = abs(df["oswLat"]) < LAT_MAX
        logging.debug("latmax_cond %s", latmax_cond.values.sum())
        ...
        cond_outlier_ww3_hs = abs(df["ww3_effective_2Dcutoff_hs"]) < 50
        logging.debug(
            "cond_outlier_ww3_hs %s", cond_outlier_ww3_hs.values.sum()
        )
        fini_bias = np.isfinite(df["bias_swh_azc_" + wv]) & (
            abs(df["bias_swh_azc_" + wv]) < 50
        )
        ...
        cond_outlier_ww3_hs = (abs(df["ww3_effective_2Dcutoff_hs"]) < 50) & (
            df["ww3_effective_2Dcutoff_hs"] > 0.3
        )
        logging.debug(
            "cond_outlier_ww3_hs %s", cond_outlier_ww3_hs.values.sum()
        )
        ...
        raise Exception("version unknown")
    logging.debug(
        "start_prior_period : %s -> stop_prior_period %s",
        start_prior_period,
        stop_prior_period,
    )
    ...
    mask_prior_period = (df["fdatedt"] >= start_prior_period64) & (
        df["fdatedt"] <= stop_prior_period64
    )
    final_mask_prior = ocean_acqui_filters & mask_prior_period
    logging.debug("final_mask_prior %s", final_mask_prior.values.sum())
    logging.debug(
        "nb pts in prior period (without extra filters) : %s",
        mask_prior_period.values.sum(),
    )
    ...
    logging.debug(
        "nb finite %s/%s",
        np.isfinite(subset_df["bias_swh_azc_" + wv].values).sum(),
    ...
    current_date_cond = (df["fdatedt"] >= start_current_month64) & (
        df["fdatedt"] <= stop_current_month64
    )
    ...
    if (
        "ww3_effective_2Dcutoff_hs" in subset_current_period
        and len(subset_current_period["ww3_effective_2Dcutoff_hs"]) > 0
    ):
        logging.debug(
            "max Hs WW3 in subset : %s",
            np.nanmax(subset_current_period["ww3_effective_2Dcutoff_hs"]),
        )
        logging.debug(
            "max Hs SAR in subset : %s",
            np.nanmax(subset_current_period["s1_effective_hs_2Dcutoff"]),
        )
        logging.debug(
            "min Hs WW3 in subset : %s",
            np.nanmin(subset_current_period["ww3_effective_2Dcutoff_hs"]),
        )
        logging.debug(
            "min Hs SAR in subset : %s",
        ...
        logging.debug(
            "bias : %s", subset_current_period["bias_swh_azc_" + wv].values
        )
        if version == "test7":
            current_mean_bias = np.mean(
                subset_current_period["bias_swh_azc_" + wv]
            ).values
            cond_sup = (
                subset_current_period["bias_swh_azc_" + wv]
                <= current_mean_bias + envelop_value
            )
            cond_inf = (
                subset_current_period["bias_swh_azc_" + wv]
                >= current_mean_bias - envelop_value
            ...
            bias_minus_2sigma = abs(
                subset_current_period["bias_swh_azc_" + wv].mean().values
                - envelop_value
            )
            bias_plus_2sigma = abs(
                subset_current_period["bias_swh_azc_" + wv].mean().values
                + envelop_value
            )
            ...
            nb_measu_inside_envelop = (
                (
                    abs(subset_current_period["bias_swh_azc_" + wv])
                    < envelop_value
                )
                .sum()
                .values
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--enddate",
        help="end of the 1 month period analysed",
        required=False,
        action="store",
        default=None,
    )
    ...
    output_file = (
        "/home1/scratch/agrouaze/kpi_1d_v2/{}/kpi_output_{}_{}_{}.txt".format(args.version, sat, wv, end_date.strftime("%Y%m%d"))
    )
    if os.path.exists(output_file) and args.overwrite is False:
        ...
        logging.info("##########")
        logging.info(
            "kpi: %s %s :  %1.3f %% (envelop %s-sigma value: %1.3f m)",
            sat,
            wv,
            kpi_v,
            ENVELOP,
        ...
        logging.info("##########")
        logging.info(
            "start_cur_month : %s stop_cur_month : %s",
            start_cur_month,
            stop_cur_month,
        )
        ...
        fid.write(
            f"{kpi_v} {start_cur_month} {stop_cur_month} {envelop_val} {nbvalused} {meanbias}\n"
        )
        fid.close()
        logging.info("output: %s", output_file)
        logging.info("done in %1.3f min", (time.time() - t0) / 60.0)
        logging.info(
            "peak memory usage: %s Mbytes",
